          ...
          {wisatas.data &&
            <Picker
              ...
              onValueChange={(itemValue, itemIndex) => setSelectedValue({
                ...
                current: itemValue,
                filter: {
                  lat: wisatas.data.filter((row) => row.nama_wisata === itemValue).map((row) => row.latitude).toString(),
                  lng: wisatas.data.filter((row) => row.nama_wisata === itemValue).map((row) => row.longitude).toString()
                }
              })
              }
            >
